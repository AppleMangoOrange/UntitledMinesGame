...
static const struct game_params mines_presets[] = {
    {9, 9, 10, true, -1, -1},   {9, 9, 35, true, -1, -1},
    {16, 16, 40, true, -1, -1}, {16, 16, 99, true, -1, -1},
#ifndef SMALL_SCREEN
    {30, 16, 99, true, -1, -1}, {30, 16, 170, true, -1, -1},
#endif
};
...
static void decode_params(game_params *params, char const *string) {
  ...
  if (*p == 'n') {
    ...
    if (params->h > 0 && params->w > 0 && params->w <= INT_MAX / params->h)
  ...
  for (xx = x - 3; xx < x + 3; xx++)
    for (yy = y - 3; yy < y + 3; yy++) {
      ...
      if (findrelpos234(ss->sets, &stmp, NULL, REL234_GE, &pos)) {
        while ((s = index234(ss->sets, pos)) != NULL && s->x == xx &&
               s->y == yy) {
          /*
...
static void known_squares(int w, int h, struct squaretodo *std,
                          signed char *grid, open_cb open, void *openctx, int x,
                          int y, int mask, bool mine) {
  int xx, yy, bit;
...
typedef struct perturbations *(*perturb_cb)(void *, signed char *, int, int,
                                            int);

static int minesolve(int w, int h, int n, signed char *grid, open_cb open,
                     perturb_cb perturb, void *ctx, random_state *rs) {
  ...
  while (1) {
    ...
    if ((s = ss_todo(ss)) != NULL) {
      ...
      if (s->mines == 0 || s->mines == bitcount16(s->mask)) {
        ...
        printf("easy\n");
#endif
        known_squares(w, h, std, grid, open, ctx, s->x, s->y, s->mask,
                      (s->mines != 0));
      ...
      for (j = 0; list[j]; j++) {
        ...
        if (swc == s->mines - s2->mines || s2wc == s2->mines - s->mines) {
          known_squares(w, h, std, grid, open, ctx, s->x, s->y, swing,
                        (swc == s->mines - s2->mines));
          known_squares(w, h, std, grid, open, ctx, s2->x, s2->y, s2wing,
      ...
      if (minesleft == 0 || minesleft == squaresleft) {
        for (i = 0; i < w * h; i++)
          if (grid[i] == -2)
            known_squares(w, h, std, grid, open, ctx, i % w, i / w, 1,
                          minesleft != 0);
            ...
            if (squaresleft > 0 &&
              ...
              for (i = 0; i < w * h; i++)
                if (grid[i] == -2) {
                  ...
                  x = i % w;
                  for (j = 0; j < nsets; j++)
                    if (setused[j] && setmunge(sets[j]->x, sets[j]->y,
                                               sets[j]->mask, x, y, 1, false)) {
                      outside = false;
                      break;
                    }
                  if (outside)
                    known_squares(w, h, std, grid, open, ctx, x, y, 1,
                                  minesleft != 0);
                }
    ...
    if (perturb) {
      ...
      if (ret) {
        ...
        for (i = 0; i < ret->n; i++) {
          ...
          list = ss_overlap(ss, ret->changes[i].x, ret->changes[i].y, 1);
  ...
  for (y = 0; y < ctx->h; y++)
    for (x = 0; x < ctx->w; x++) {
      ...
      if (grid[y * ctx->w + x] != -2) {
        ...
        for (dy = -1; dy <= +1; dy++)
          for (dx = -1; dx <= +1; dx++)
            if (x + dx >= 0 && x + dx < ctx->w && y + dy >= 0 &&
                y + dy < ctx->h && grid[(y + dy) * ctx->w + (x + dx)] != -2) {
              sqlist[n].type = 1;
              break;
            }
      }
...
#ifdef GENERATION_DIAGNOSTICS
      printf(" (%d,%d)", setlist[index] % ctx->w, setlist[index] / ctx->w);
#endif
    }
#ifdef GENERATION_DIAGNOSTICS
    printf("\n");
  ...
  if (setlist) {
    ...
    for (dy = 0; dy < 3; dy++)
      for (dx = 0; dx < 3; dx++)
        if (mask & (1 << (dy * 3 + dx))) {
          int currval =
              (ctx->grid[(sety + dy) * ctx->w + (setx + dx)] ? +1 : -1);
          if (dset == -currval) {
        ...
        if (x + dx >= 0 && x + dx < ctx->w && y + dy >= 0 && y + dy < ctx->h &&
          ...
          if (dx == 0 && dy == 0) {
            ...
            if (delta > 0) {
              ...
              for (dy2 = -1; dy2 <= +1; dy2++)
                for (dx2 = -1; dx2 <= +1; dx2++)
                  if (x + dx2 >= 0 && x + dx2 < ctx->w && y + dy2 >= 0 &&
                      y + dy2 < ctx->h &&
...
static char *describe_layout(bool *grid, int area, int x, int y,
  ...
  p = ret +
      sprintf(ret, "%d,%d,%s", x, y, obfuscate ? "m" : "u"); /* 'm' == masked */
...
static char *new_game_desc(const game_params *params, random_state *rs,
  ...
  if (!interactive) {
    ...
    grid = new_mine_layout(params->w, params->h, params->n, x, y,
                           params->unique, rs, &desc);
    sfree(grid);
    return desc;
  } else {
    char *rsdesc, *desc;
    ...
    sprintf(desc, "r%d,%c,%s", params->n, (char)(params->unique ? 'u' : 'a'),
            rsdesc);
    sfree(rsdesc);
    return desc;
  }
...
static int open_square(game_state *state, int x, int y) {
  ...
  if (!state->layout->mines) {
    ...
    char *desc, *privdesc;
    state->layout->mines =
        new_mine_layout(w, h, state->layout->n, x, y, state->layout->unique,
                        state->layout->rs, &desc);
     ...
     */
    privdesc = desc;
    while (*privdesc && isdigit((unsigned char)*privdesc))
      privdesc++;
    if (*privdesc == ',')
      privdesc++;
    while (*privdesc && isdigit((unsigned char)*privdesc))
    ...
    for (yy = 0; yy < h; yy++)
      for (xx = 0; xx < w; xx++)
        if (state->grid[yy * w + xx] == -10) {
          ...
          for (dx = -1; dx <= +1; dx++)
            for (dy = -1; dy <= +1; dy++)
              if (xx + dx >= 0 && xx + dx < state->w && yy + dy >= 0 &&
                  yy + dy < state->h &&
...
static const char *current_key_label(const game_ui *ui, const game_state *state,
                                     int button) {
  int cx = ui->cur_x, cy = ui->cur_y;
  int v = state->grid[cy * state->w + cx];

  if (state->dead || state->won || !ui->cur_visible)
    return "";
  ...
  if (button == CURSOR_SELECT) {
    ...
    for (dy = -1; dy <= +1; dy++)
      for (dx = -1; dx <= +1; dx++)
        if (cx + dx >= 0 && cx + dx < state->w && cy + dy >= 0 &&
            cy + dy < state->h) {
...
static char *interpret_move(const game_state *from, game_ui *ui,
  ...
  if (IS_CURSOR_MOVE(button))
    return move_cursor(button, &ui->cur_x, &ui->cur_y, from->w, from->h, false,
                       &ui->cur_visible);
  ...
  if (button == LEFT_BUTTON || button == LEFT_DRAG || button == MIDDLE_BUTTON ||
      button == MIDDLE_DRAG) {
...
uncover: {
  ...
  if (from->grid[cy * from->w + cx] > 0 && ui->validradius == 1) {
    ...
    for (dy = -1; dy <= +1; dy++)
      for (dx = -1; dx <= +1; dx++)
        if (cx + dx >= 0 && cx + dx < from->w && cy + dy >= 0 &&
            cy + dy < from->h) {
...
static game_state *execute_move(const game_state *from, const char *move) {
  ...
  if (!strcmp(move, "S")) {
      ...
      for (yy = 0; yy < ret->h; yy++)
        for (xx = 0; xx < ret->w; xx++) {
          ...
          if (ret->layout->mines[yy * ret->w + xx]) {
            ...
            for (dx = -1; dx <= +1; dx++)
              for (dy = -1; dy <= +1; dy++)
                if (xx + dx >= 0 && xx + dx < ret->w && yy + dy >= 0 &&
                    yy + dy < ret->h &&
      ...
      for (yy = 0; yy < ret->h; yy++)
        for (xx = 0; xx < ret->w; xx++) {
          ...
          if ((ret->grid[pos] == -2 || ret->grid[pos] == -3) &&
            ...
            ret->grid[pos] = 64;
          } else if (ret->grid[pos] == -1 && !ret->layout->mines[pos]) {
            ret->grid[pos] = 66;
          }
        }
    }
    ...
    while (*move) {
      if (move[0] == 'F' && sscanf(move + 1, "%d,%d", &cx, &cy) == 2 &&
          cx >= 0 && cx < from->w && cy >= 0 && cy < from->h &&
          (ret->grid[cy * from->w + cx] == -1 ||
           ret->grid[cy * from->w + cx] == -2)) {
        ret->grid[cy * from->w + cx] ^= (-2 ^ -1);
      } else if (move[0] == 'O' && sscanf(move + 1, "%d,%d", &cx, &cy) == 2 &&
                 cx >= 0 && cx < from->w && cy >= 0 && cy < from->h) {
        open_square(ret, cx, cy);
      } else if (move[0] == 'C' && sscanf(move + 1, "%d,%d", &cx, &cy) == 2 &&
                 cx >= 0 && cx < from->w && cy >= 0 && cy < from->h) {
        int dx, dy;

        for (dy = -1; dy <= +1; dy++)
          for (dx = -1; dx <= +1; dx++)
            if (cx + dx >= 0 && cx + dx < ret->w && cy + dy >= 0 &&
                cy + dy < ret->h &&
...
static void draw_tile(drawing *dr, game_drawstate *ds, int x, int y, int v,
  ...
  if (v < 0) {
    ...
    if (v == -22 || v == -23 || v == -24) {
      ...
      coords[0] = x;
      coords[1] = y;
      draw_polygon(dr, coords, 3, COL_HIGHLIGHT ^ hl, COL_HIGHLIGHT ^ hl);

      draw_rect(dr, x + HIGHLIGHT_WIDTH, y + HIGHLIGHT_WIDTH,
                TILE_SIZE - 2 * HIGHLIGHT_WIDTH,
                TILE_SIZE - 2 * HIGHLIGHT_WIDTH, bg);
    }
...
#undef SETCOORD
      ...
      draw_text(dr, x + TILE_SIZE / 2, y + TILE_SIZE / 2, FONT_VARIABLE,
                TILE_SIZE * 6 / 8, ALIGN_VCENTRE | ALIGN_HCENTRE, COL_QUERY,
                "?");
    } else if (v == -4) {
      /*
    ...
    draw_rect(dr, x, y, TILE_SIZE, TILE_SIZE,
              (v == 65                ? COL_BANG
               : bg == COL_BACKGROUND ? COL_BACKGROUND2
                                      : bg));
    draw_line(dr, x, y, x + TILE_SIZE - 1, y, COL_LOWLIGHT);
    draw_line(dr, x, y, x, y + TILE_SIZE - 1, COL_LOWLIGHT);
    ...
    if (v > 0 && v <= 8) {
      ...
      str[1] = '\0';
      draw_text(dr, x + TILE_SIZE / 2, y + TILE_SIZE / 2, FONT_VARIABLE,
                TILE_SIZE * 7 / 8, ALIGN_VCENTRE | ALIGN_HCENTRE,
                (COL_1 - 1) + v, str);
      ...
      if (v == 66) {
        ...
        int dx;
        for (dx = -1; dx <= +1; dx++) {
          draw_line(dr, x + 3 + dx, y + 2, x + TILE_SIZE - 3 + dx,
                    y + TILE_SIZE - 2, COL_CROSS);
          draw_line(dr, x + TILE_SIZE - 3 + dx, y + 2, x + 3 + dx,
                    y + TILE_SIZE - 2, COL_CROSS);
        }
      }
    }
...
static void game_redraw(drawing *dr, game_drawstate *ds,
                        ...
                        int dir, const game_ui *ui, float animtime,
                        float flashtime) {
  int x, y;
    ...
    if (state->dead) {
      ...
      if (safe_closed > 0 && safe_closed <= 9) {
        ...
        if (safe_closed == 1) {
          ...
          sprintf(statusbar + strlen(statusbar), " (%d safe squares remain)",
                  safe_closed);
        }
      }
    }
...
static void game_get_cursor_location(const game_ui *ui,
                                     ...
                                     const game_state *state,
                                     const game_params *params, int *x, int *y,
                                     int *w, int *h) {
  if (ui->cur_visible) {
    *x = COORD(ui->cur_x);
    *y = COORD(ui->cur_y);
...
int main(int argc, char **argv) {
  ...
  *desc++ = '\0';
  ...
  printf("%s:%s\n", id,
         describe_layout(s->layout->mines, p->w * p->h, x, y, (*desc != 'm')));
